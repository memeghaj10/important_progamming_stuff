...
public:
    ...
    int sumOfLongRootToLeafPath(Node *root)
    {
       ...
       while(!q.empty()) {
           ...
           int curr_sum=q.front().second.first;
           int curr_level=q.front().second.second;
           
           q.pop();
           
            if(curr_level>max_len) {
                max_sum=curr_sum;
                max_len=curr_level;
            }
            
            else if(curr_level==max_len&&curr_sum>max_sum) {
                max_sum=curr_sum;
            }
            
            if(r->left) {
                q.push({r->left,{curr_sum+r->left->data,curr_level+1}});
            }
            
            if(r->right) {
                q.push({r->right,{curr_sum+r->right->data,curr_level+1}});
            }
